...
consoleintr(int (*getc)(void))
{
  ...
  while((c = getc()) >= 0){
    ...
    case KEY_UP:
        {
          if ((hist.e <= HISTORY_SIZE  && hist.r == 0)
          || ((hist.e % HISTORY_SIZE) == (hist.r-1) % HISTORY_SIZE))
            break;
          ...
          {
            char character = hist.buf[hist.r % HISTORY_SIZE][i];
            if ((character == NULL) || (character == '\n'))
              break;
            input.buf[input.e % INPUT_BUF] = character;
            input.e++;
            consputc(character);
          }
        }
    ...
    case KEY_DN:
        {
          if((hist.r % HISTORY_SIZE) == (hist.e % HISTORY_SIZE))
            break;
          else
            hist.r++;
